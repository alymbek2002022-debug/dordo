...
*🗑️ /remove_daily* - Удалить ежедневную отправку
    Пример: `/remove_daily 09:00`    git add .
    git commit -m "All handlers in place, including add_daily and server_time"
    git push
...
def scheduled_sender():
    ...
    while not stop_scheduler:
        try:
            group_id = messages_storage.get('group_id')
            ...
            # Сбрасываем набор отправленных ключей при смене минуты
            minute_key = now.strftime('%Y-%m-%d %H:%M')
            if minute_key != last_checked_minute:
                last_sent_times.clear()
            ...
            if group_id is None:
                # Логируем отсутствие настроенной группы один раз в минуту
                if not warned_no_group and now.second == 0:
                    logger.warning("Группа для отправки не установлена (messages_storage['group_id'] is None)")
...
def main() -> None:
    ...
    try:
        try:
            ...
            # Останавливаем планировщик при фатальной ошибке polling
            stop_scheduler = True
    finally:
        stop_scheduler = True
